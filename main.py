...
def home_text(user_id: int) -> str:
    ...
    uid = escape(str(s.get("firebase_uid") or "-"))
...
async def login_password(message: Message, state: FSMContext):
    ...
    try:
        ...
        player_name = record.get("Name") if isinstance(record, dict) else None
        ...
        await loading.edit_text(
            ...
            "━━━━━━━━━━━━━━━━━━\n"
            f"📧 {escape(auth['email'])}\n"
            f"👤 {escape(str(player_name or 'Unknown'))}\n"
            f"🆔 <code>{escape(auth['firebase_uid'])}</code>\n\n"
...
def _record_value(record, *keys, default="-"):
    """Return the first non-empty value from a CPM record."""
...
def _account_info_text(s: dict) -> str:
    record = s.get("record") or {}

    name = _record_value(
        record, "Name", "name", "PlayerName", "playerName",
        default=s.get("player_name") or "Unknown"
    )
    player_id = _record_value(
        record, "PlayerID", "PlayerId", "playerId", "player_id",
        "GameUID", "GameUid", "gameUID", "UID", "uid"
    )
    money = _record_value(record, "Money", "money", "Cash", "cash")
    coins = _record_value(record, "Coins", "coins")
    level = _record_value(record, "Level", "level")
    cars = _record_value(record, "Cars", "cars", "CarCount", "carCount")
    clothes = _record_value(
        record, "Clothes", "clothes", "ClothesCount", "clothesCount"
    )
    tuning = _record_value(
        record, "Tunings", "tunings", "Tuning", "tuning"
    )

    return f"""<b>👤 CPM1 ACCOUNT INFO</b>
<i>Berikut adalah informasi akun CPM1 Anda.</i>
━━━━━━━━━━━━━━━━━━━━━━━━

<b>👤 INFORMASI AKUN</b>
┌──────────────────────────────
│ 👤 <b>Nama Akun CPM</b>
│    <code>{escape(str(name))}</code>
│ 🆔 <b>Player ID (UID)</b>
│    <code>{escape(str(player_id))}</code>
│ 📧 <b>Email</b>
│    <code>{escape(str(s['email']))}</code>
│ 🟢 <b>Status</b>
│    <b>Logged In</b>
│ 🔥 <b>Firebase UID</b>
│    <code>{escape(str(s['firebase_uid']))}</code>
└──────────────────────────────

<b>🎮 DATA DALAM GAME</b>
┌──────────────────────────────
│ 💵 <b>Uang</b>        : <code>{escape(str(money))}</code>
│ 🪙 <b>Coins</b>       : <code>{escape(str(coins))}</code>
│ ⭐ <b>Level</b>       : <code>{escape(str(level))}</code>
│ 🚗 <b>Mobil</b>       : <code>{escape(str(cars))}</code>
│ 👕 <b>Pakaian</b>     : <code>{escape(str(clothes))}</code>
│ 🔧 <b>Tuning</b>      : <code>{escape(str(tuning))}</code>
└──────────────────────────────

<i>ℹ️ Data yang tidak dikirim server CPM1 ditampilkan sebagai '-'.</i>"""
