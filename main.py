...
def kb_cancel():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Cancel Login", callback_data="cancel")]
    ])

def kb_login_skip(field):
    if field == "password":
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⏭️ Skip Password", callback_data="login_skip_password")],
            [InlineKeyboardButton(text="❌ Cancel Login", callback_data="cancel")]
        ])
    if field == "email":
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⏭️ Skip Email", callback_data="login_skip_email")],
            [InlineKeyboardButton(text="🔙 Back", callback_data="login_back_password"),
...
def login_progress(step, title, hint):
    ...
    return (
        f"<b>🔐 {title}</b>\n"
        f"<code>{bars.get(step, '⬜⬜⬜')}</code>  <b>{step}/3</b>\n\n"
        f"{hint}\n\n"
        f"<i>Data login hanya digunakan untuk memuat akun pada sesi ini.</i>"
    )
